...
function CommunityDetailPage(page){
    ...
    $.ajax({
        ...
        success : function(data) {
            ...
            var paging = data.paging;
            communityDetailChangePage(keyword, paging, lstSelectCampsiteReview, reviewBoolean);
        },
        error : function(data) {
            console.log("data failed : " + data);
            console.log("page"+page);
...
function communityDetailChangePage(keyword, paging, lstSelectCampsiteReview, reviewBoolean) {
    $('.community_aticle').children('#blogUl').remove();
    var communityRow;
    communityRow = "<ul id='blogUl'>"
    for (var i = 0; i < 5; i++) {
